...
export class LoginCommand {
    ...
    try {
      ...
      if (clientId != null && clientSecret != null) {
        if (!clientId.startsWith("user")) {
          return Response.error("Invalid API Key; Team API Key currently not supported");
        }
        try {
  ...
  private async updateWeakPassword(currentPassword: string) {
    ...
    if (!this.canInteract) {
      ...
      return Response.error(
        new MessageResponse(
          "Your master password does not meet one or more of your team policies. In order to access the vault, you must update your master password now via the web vault. You have been logged out.",
          null
        )
      );
    }
    ...
    try {
      const { newPasswordHash, newEncKey, hint } = await this.collectNewMasterPasswordDetails(
        "Your master password does not meet one or more of your team policies. In order to access the vault, you must update your master password now."
      );
  ...
  private async updateTempPassword() {
    ...
    if (!this.canInteract) {
      ...
      return Response.error(
        new MessageResponse(
          "An team administrator recently changed your master password. In order to access the vault, you must update your master password now via the web vault. You have been logged out.",
          null
        )
      );
    }
    ...
    try {
      const { newPasswordHash, newEncKey, hint } = await this.collectNewMasterPasswordDetails(
        "An team administrator recently changed your master password. In order to access the vault, you must update your master password now."
      );
  ...
  private async handleCaptchaRequired(
    ...
    const badCaptcha = Response.badRequest(
      "Your authentication request has been flagged and will require user interaction to proceed.\n" +
        ...
        "(https://bravurasafe.com/help/cli-auth-challenges)"
    );
