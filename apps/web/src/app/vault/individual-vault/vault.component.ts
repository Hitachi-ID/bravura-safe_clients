...
import { OrganizationService, isMember } from "@bitwarden/common/admin-console/abstractions/organization/organization.service.abstraction";
...
export class VaultComponent implements OnInit, OnDestroy {
    ...
    firstSetup$
      ...
      .subscribe(
        ([
          ...
          this.allOrganizations = allOrganizations.sort((a, b) => a.name.localeCompare(b.name));
  ...
  async copy(cipher: CipherView, field: "username" | "password" | "totp") {
    ...
    if (value == null || !cipher.viewPassword) {
      return;
    }
  ...
  async launch(cipher: CipherView) {
    if (cipher.login.uri) {
