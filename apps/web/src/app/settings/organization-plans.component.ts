...
export class OrganizationPlansComponent implements OnInit, OnDestroy {
      ...
      const doSubmit = async (): Promise<string> => {
        ...
        if (this.createOrganization) {
          ...
          const collection = await this.cryptoService.encrypt(
            this.i18nService.t("defaultCollection") + " - " + this.formGroup.controls.name.value,
            shareKey[1]
          );
  ...
  private async createCloudHosted(
    ...
    request.businessName = this.formGroup.controls.businessOwned.value ? this.formGroup.controls.businessName.value : null;
